...
"""
@permission_required('esotre.change_product')
def product_update(request, pk):
    product = get_object_or_404(Product, pk=pk)
    form = ProductForm(request.POST or None, isinstance=prodcut)

    if request.POST and form.is_valid():
        form.save()
        messages.success(request, '產品已變更')

    return render(request, 'esostre/product_form.html', {'form': form})
"""
class ProductUpdate(PermissionRequiredMixin, generic.UpdateView):
    permission_required = 'estore.change_product'
    model = Product
    fields = ('title', 'description', 'quantity', 'prise', 'image')
